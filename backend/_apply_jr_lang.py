import json, os
B = "."
d = json.load(open("_junior_lang_crawl.json", encoding="utf-8"))
...
def pick_lang(candidates):
    for c in candidates:
        ...
        if any(k in u for k in ["klec","kli","korean","한국어","어학","language","연수"]) and any(k in t for k in ["한국어","어학","language","korean","연수","교육"]):
            return c["url"]
    for c in candidates:
        u = c["url"].lower(); t = c["text"].lower()
        if any(k in u for k in ["klec","kli","korean","한국어","어학","language"]):
            return c["url"]
    return None
